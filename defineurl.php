...
$defaultid = required_param('defaultid', PARAM_INT); // Ddefault id.
require_login();
...
$currenturl = new moodle_url('/filter/gurls/defineurl.php', array('defaultid' => $defaultid));

$PAGE->set_context($context);
$PAGE->set_url($CFG->wwwroot . '/admin/settings.php?section=filtersettinggurls');
$PAGE->set_pagelayout('standard');
$PAGE->set_heading(get_string('defineurl', 'filter_gurls'));
$PAGE->set_title(get_string('defineurl', 'filter_gurls'));
$PAGE->navbar->add(get_string('filtersettings', 'filter_gurls'), $navurl);
$PAGE->navbar->add(get_string('gurlpanel', 'filter_gurls'), $navurl2);
$PAGE->navbar->add(get_string('defineurl', 'filter_gurls'));

$output = $PAGE->get_renderer('filter_gurls');

echo $OUTPUT->header();
$output->filter_gurl_urltable($defaultid);
echo $OUTPUT->footer();